...
const ChallanModule = (() => {
  ...
  async function viewChallan(id) {
    ...
    App.setHeaderActions(`
      ...
      <button class="header-btn" onclick="PrintModule.exportChallanPDF(${id})" title="Export PDF">📄</button>
    ...
    document.getElementById('challan-view-content').innerHTML = `
      ...
      <div style="display:flex;gap:8px;padding-bottom:8px" class="no-print">
        <button class="btn btn-primary flex-1" onclick="PrintModule.exportChallanPDF(${id})">📄 Export PDF (4 copies)</button>
  ...
  function printChallan(id) {
    PrintModule.exportChallanPDF(id);
  }
